...
const DEFAULT_FOOTER_NOTICE =
  "Entwickelt von Daniel Soro (Kundensupport bei 4NetPlayers) mit Unterstützung von KI zur Erleichterung seiner Supportarbeit. Privat unter lynxnode.it betrieben; kein offizielles Angebot von 4NetPlayers.";
const FOOTER_NOTICE = Deno.env.get("FOOTER_NOTICE")?.trim() ||
...
async function queryServer(request: QueryRequest): Promise<QueryResult> {
  if (
    ["satisfactory", "fivem", "palworld-rest", "scum"].includes(
      request.transport,
    )
  ) {
...
function styles(): string {
  return `<style>
:root{color-scheme:dark;font-family:Montserrat,Inter,ui-sans-serif,system-ui,-apple-system,"Segoe UI",sans-serif;background:#181817;color:#fff;--maire:#181817;--maire-soft:#222221;--maire-panel:#242422;--maire-panel-2:#2b2b29;--red:#eb1b2e;--red-bright:#ff3044;--white:#fff;--grey:#d9d9d8;--muted:#a9a9a5;--line:rgba(217,217,216,.17);--ok:#75e3af}*{box-sizing:border-box}body{min-height:100vh;margin:0;background:radial-gradient(circle at 86% -12%,rgba(235,27,46,.2),transparent 34%),radial-gradient(circle at 0 15%,rgba(217,217,216,.07),transparent 31%),var(--maire);padding:24px}body:before{content:"";position:fixed;inset:0;pointer-events:none;opacity:.22;background-image:linear-gradient(rgba(255,255,255,.025) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,.025) 1px,transparent 1px);background-size:48px 48px;mask-image:linear-gradient(to bottom,black,transparent 78%)}.shell{position:relative;width:min(100%,1180px);margin:0 auto}.narrow{width:min(100%,620px);min-height:calc(100vh - 48px);display:grid;place-items:center}.panel{border:1px solid var(--line);border-radius:18px;background:linear-gradient(145deg,rgba(43,43,41,.97),rgba(31,31,30,.97));box-shadow:0 24px 70px rgba(0,0,0,.38);padding:clamp(22px,4vw,40px)}.login-panel{padding:clamp(28px,6vw,54px)}.brand-lockup{display:flex;align-items:center;gap:18px}.brand-logo{display:block;width:min(240px,42vw);height:auto}.login-panel .brand-logo{width:min(250px,70vw)}.eyebrow{margin:0 0 9px;color:var(--red-bright);font-size:.72rem;font-weight:800;letter-spacing:.16em;text-transform:uppercase}.muted{color:var(--muted);line-height:1.65}.error{color:#ff9ba5;min-height:1.4em}.stack{display:grid;gap:16px}label{display:grid;gap:8px;color:var(--grey);font-size:.82rem;font-weight:700;letter-spacing:.01em}.label-row{display:flex;align-items:center;gap:7px}.help-dot{position:relative;display:inline-grid;place-items:center;width:17px;height:17px;border:1px solid rgba(217,217,216,.42);border-radius:50%;color:var(--grey);font-size:.68rem;font-weight:850;line-height:1;cursor:help}.help-dot::after{content:attr(data-tooltip);position:absolute;left:50%;bottom:calc(100% + 10px);z-index:10;width:min(320px,calc(100vw - 40px));padding:10px 12px;border:1px solid rgba(217,217,216,.24);border-radius:9px;background:#222221;color:var(--white);font-size:.76rem;font-weight:500;line-height:1.45;text-align:left;box-shadow:0 14px 30px rgba(0,0,0,.3);opacity:0;pointer-events:none;transform:translate(-50%,4px);transition:opacity .16s ease,transform .16s ease}.help-dot:hover::after,.help-dot:focus-visible::after{opacity:1;transform:translate(-50%,0)}.help-dot:focus-visible{outline:2px solid var(--red);outline-offset:2px}input,select,textarea,button{width:100%;min-height:46px;padding:0 13px;border:1px solid rgba(217,217,216,.24);border-radius:9px;background:#181817;color:var(--white);font:inherit;transition:border-color .18s ease,box-shadow .18s ease,transform .18s ease}input::placeholder,textarea::placeholder{color:#777772}input:focus,select:focus,textarea:focus{outline:none;border-color:var(--red);box-shadow:0 0 0 3px rgba(235,27,46,.16)}textarea{padding-top:12px;min-height:120px;resize:vertical}button{border:1px solid var(--red);background:var(--red);color:var(--white);cursor:pointer;font-weight:800;box-shadow:0 9px 22px rgba(235,27,46,.17)}button:hover{background:var(--red-bright);transform:translateY(-1px)}button:disabled{opacity:.65;cursor:wait;transform:none}.topbar{display:flex;align-items:center;justify-content:space-between;gap:20px;margin:8px 0 24px}.topbar h1{margin:0;font-size:clamp(1.35rem,3.4vw,2.1rem);letter-spacing:-.035em}.topbar button{width:auto;padding:0 16px}.tabs{display:flex;gap:8px;margin:0 0 18px}.tabs button{width:auto;background:transparent;color:var(--muted);border-color:transparent;box-shadow:none}.tabs button:hover{color:var(--white);background:rgba(255,255,255,.06);transform:none}.tabs button.active{background:var(--red);border-color:var(--red);color:var(--white);box-shadow:0 8px 20px rgba(235,27,46,.17)}.tab{display:none}.tab.active{display:block}.form-grid{display:grid;grid-template-columns:1fr 180px;gap:14px}.form-grid.three{grid-template-columns:1fr 180px 220px}.result{margin-top:24px;border-top:1px solid var(--line);padding-top:22px}.status{font-weight:850}.status-banner{display:flex;align-items:center;justify-content:space-between;gap:14px;padding:14px 16px;border:1px solid;border-radius:10px;font-weight:850}.status-banner.online{color:#f3fff8;background:linear-gradient(100deg,#16844f,#24a866);border-color:#4fda8d}.status-banner.offline{color:#fff0f2;background:linear-gradient(100deg,#8d1e2a,#b82b3a);border-color:#f06b79}.status-label{font-size:1.06rem;letter-spacing:.01em}.status-ping{font-variant-numeric:tabular-nums;font-weight:750}.protocol-meta{margin-top:10px;color:#92928d;font-size:.82rem}.online{color:var(--ok)}.offline,.warning{color:#ff8c98}.checking{color:#f1c978}.meta{color:#92928d}.details{display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:10px;margin-top:18px}.detail{padding:13px 14px;border:1px solid rgba(217,217,216,.1);border-radius:10px;background:rgba(24,24,23,.7);overflow-wrap:anywhere}.detail small{display:block;margin-bottom:5px;color:#8e8e88}.section{margin-top:20px}.section h3{margin:0 0 10px;font-size:1rem}.table-wrap{overflow:auto;border:1px solid var(--line);border-radius:10px}.table{width:100%;border-collapse:collapse;min-width:420px}.table th,.table td{text-align:left;padding:10px 12px;border-bottom:1px solid rgba(217,217,216,.11)}.table th{color:var(--grey);font-size:.76rem;text-transform:uppercase;letter-spacing:.05em}.raw{white-space:pre-wrap;word-break:break-word;max-height:300px;overflow:auto;padding:14px;border-radius:10px;background:#111110;color:var(--grey)}.hint{margin:10px 0 0;color:#8f8f89;font-size:.82rem}.accent-rule{height:3px;width:54px;margin:20px 0;background:var(--red);border-radius:99px}.subtle{border-left:3px solid var(--red);padding-left:13px}.legal-footer{margin:22px auto 0;width:min(100%,760px);color:#85857f;font-size:.72rem;line-height:1.55;text-align:center;white-space:pre-line}@media(max-width:760px){body{padding:14px}.form-grid,.form-grid.three{grid-template-columns:1fr}.topbar{align-items:flex-start;flex-direction:column}.topbar button{width:100%}.brand-lockup{align-items:flex-start;flex-direction:column;gap:12px}.brand-logo{width:min(220px,72vw)}}h1,h2,h3{line-height:1.08}h2{letter-spacing:-.025em}
</style>`;
}

function footerHtml(): string {
  return '<footer class="legal-footer">' + escapeHtml(FOOTER_NOTICE) +
    "</footer>";
}

function loginPage(): string {
  return `<!doctype html><html lang="de"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="theme-color" content="#181817"><link rel="icon" type="image/svg+xml" href="/assets/4np-globe-white.svg"><title>Support Query Tool – Anmeldung</title>${styles()}</head><body><main class="shell narrow"><section class="panel login-panel"><div class="brand-lockup"><img class="brand-logo" src="/assets/4netplayers-horizontal-white.svg" alt="4NetPlayers"><div><p class="eyebrow">Support Operations</p><h1>Server Diagnostic Console</h1></div></div><div class="accent-rule"></div><p class="muted">Der Support-Bereich ist passwortgeschützt. Melde dich an, um Serverabfragen und RCON-Befehle sicher auszuführen.</p><form id="login-form" class="stack"><label>Support-Passwort<input id="password" type="password" autocomplete="current-password" required></label><button type="submit">Anmelden</button><p id="error" class="error" aria-live="polite"></p></form></section>${footerHtml()}</main><script>document.querySelector('#login-form').addEventListener('submit',async(e)=>{e.preventDefault();const error=document.querySelector('#error');error.textContent='';const response=await fetch('/api/auth/login',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({password:document.querySelector('#password').value})});const data=await response.json();if(response.ok){location.href='/';}else{error.textContent=data.message||'Anmeldung fehlgeschlagen.';}});</script></body></html>`;
}

function appScript(): string {
  return `const $=(s)=>document.querySelector(s);const escape=(v)=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'})[c]);document.querySelectorAll('[data-tab]').forEach(b=>b.addEventListener('click',()=>{document.querySelectorAll('[data-tab]').forEach(x=>x.classList.toggle('active',x===b));document.querySelectorAll('.tab').forEach(x=>x.classList.toggle('active',x.id==='tab-'+b.dataset.tab));}));$('#logout').addEventListener('click',async()=>{await fetch('/api/auth/logout',{method:'POST'});location.href='/';});function detailCards(fields){return Object.entries(fields||{}).filter(([,v])=>v!==undefined&&v!==null&&v!=='').map(([k,v])=>'<div class="detail"><small>'+escape(k)+'</small><span>'+escape(typeof v==='object'?JSON.stringify(v):v)+'</span></div>').join('');}function table(rows){if(!rows?.length)return '<p class="hint">Der Server hat keine Einträge geliefert.</p>';const keys=[...new Set(rows.flatMap(r=>Object.keys(r)))];return '<div class="table-wrap"><table class="table"><thead><tr>'+keys.map(k=>'<th>'+escape(k)+'</th>').join('')+'</tr></thead><tbody>'+rows.map(r=>'<tr>'+keys.map(k=>'<td>'+escape(r[k])+'</td>').join('')+'</tr>').join('')+'</tbody></table></div>';}function showQuery(data){const online=data.online;const statusClass=online?'online':'offline';const isScumResult=data.transport?.startsWith('SCUM');const displayPing=data.serverPingMs??(isScumResult?undefined:data.latencyMs);let html='<div class="status-banner '+statusClass+'"><span class="status-label">'+(online?'Online':'Nicht erreichbar')+'</span>'+(displayPing!==undefined?'<span class="status-ping">'+escape(data.serverPingMs!==undefined?'Server-Ping':'Laufzeit')+': '+escape(displayPing)+' ms</span>':'')+'</div><div class="protocol-meta">'+escape(data.transport||'')+(data.serverPingMs!==undefined&&data.latencyMs!==undefined?' · Masterserver-Abfrage: '+escape(data.latencyMs)+' ms':'')+'</div>';if(!online&&data.message)html+='<p class="hint">'+escape(data.message)+'</p>';if(data.fields)html+='<div class="details">'+detailCards(data.fields)+'</div>';if(data.players)html+='<div class="section"><h3>Spieler ('+data.players.length+')</h3>'+table(data.players)+'</div>';if(data.rules)html+='<div class="section"><h3>Regeln ('+data.rules.length+')</h3>'+table(data.rules.map(x=>({Schlüssel:x.key,Wert:x.value})))+'</div>';if(data.warnings?.length)html+='<div class="section"><h3>Adapter-Hinweise</h3>'+table(data.warnings.map(x=>({Hinweis:x})))+'</div>';if(data.attempts?.length)html+='<div class="section"><h3>Automatische Versuche</h3>'+table(data.attempts.map(x=>({Protokoll:x.transport,Ergebnis:x.message})))+'</div>';$('#query-result').innerHTML=html;$('#query-result').classList.add('visible');}$('#query-form').addEventListener('submit',async e=>{e.preventDefault();const button=$('#query-submit');button.disabled=true;button.textContent='Abfrage läuft …';$('#query-result').classList.add('visible');$('#query-result').innerHTML='<span class="status checking">Prüfung läuft …</span>';try{const r=await fetch('/api/query',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({host:$('#query-host').value.trim(),port:Number($('#query-port').value),transport:$('#query-transport').value})});showQuery(await r.json());}catch{$('#query-result').innerHTML='<span class="status offline">Anfrage fehlgeschlagen</span>';}finally{button.disabled=false;button.textContent='Server abfragen';}});$('#rcon-form').addEventListener('submit',async e=>{e.preventDefault();const button=$('#rcon-submit');button.disabled=true;button.textContent='Sende …';$('#rcon-result').classList.add('visible');$('#rcon-result').innerHTML='<span class="status checking">Verbinde mit RCON …</span>';try{const r=await fetch('/api/rcon',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({host:$('#rcon-host').value.trim(),port:Number($('#rcon-port').value),protocol:$('#rcon-protocol').value,password:$('#rcon-password').value,command:$('#rcon-command').value})});const d=await r.json();$('#rcon-result').innerHTML='<div><span class="status '+(r.ok?'online':'offline')+'">'+(r.ok?'Antwort erhalten':'RCON-Fehler')+'</span> <span class="meta">'+escape(d.protocol||'')+' · '+escape(d.target||'')+(d.latencyMs?' · '+d.latencyMs+' ms':'')+'</span></div><p class="muted">'+escape(d.message||'')+'</p>'+(d.response!==undefined?'<div class="section"><h3>Serverausgabe</h3><pre class="raw">'+escape(d.response)+'</pre></div>':'');}catch{$('#rcon-result').innerHTML='<span class="status offline">RCON-Anfrage fehlgeschlagen</span>';}finally{button.disabled=false;button.textContent='Befehl senden';}});`;
}
...
const STATIC_ASSETS: Record<string, { file: URL; contentType: string }> = {
  "/assets/4np-globe-white.svg": {
    file: new URL("../public/4np-globe-white.svg", import.meta.url),
    contentType: "image/svg+xml",
  },
  "/assets/4netplayers-horizontal-white.svg": {
    file: new URL(
      "../public/4netplayers-horizontal-white.svg",
      import.meta.url,
    ),
    contentType: "image/svg+xml",
  },
...
async function staticAsset(pathname: string): Promise<Response | undefined> {
  ...
  try {
    return new Response(await Deno.readFile(asset.file), {
      headers: {
...
async function handle(request: Request): Promise<Response> {
  ...
  if (request.method === "GET" && url.pathname.startsWith("/assets/")) {
    const response = await staticAsset(url.pathname);
